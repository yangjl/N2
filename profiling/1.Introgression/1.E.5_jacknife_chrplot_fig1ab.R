...
info2$lowland <- info2$lowland - info2$mex.y
info2[info2$lowland < 0, ]$lowland <- 0
...
info2$highland <- 10*info2$highland
info2$lowland <- 10*info2$lowland
...
tab <- ddply(info3, .(bid, chr, pos), summarise,
             highland = mean(highland, na.rm=TRUE),
             lowland = mean(lowland, na.rm=TRUE))

#out <- subset(tab, mex > 2)
out$pos <- out$pos * BINSIZE
write.table(out, "graphs/table_introgress_regions_10kb_highlow.csv", sep=",", row.names =FALSE, quote=FALSE)

chrplot(tab=tab, val="highland", bin=BINSIZE, cutoff=2, cols = rep(c("slateblue", "cyan4"), 5))

pdf("graphs/int_res_chrplot_highland.pdf", width=10, height=6)
chrplot(tab=tab, val="highland", bin=BINSIZE, cutoff=2, cols = c("grey", "cyan4"), 
        xlab="Chromosome Position (10-kb)", main="Mexicana introgression in Highland Group")
dev.off()

pdf("graphs/int_res_chrplot_lowland.pdf", width=10, height=6)
chrplot(tab=tab, val="lowland", bin=BINSIZE, cutoff=2, cols = c("grey", "cyan4"), 
        xlab="Chromosome Position (10-kb)", main="Mexicana introgression in Lowland Group")
dev.off()
